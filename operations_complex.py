def Insert_Numbers():
    ...
    operation = input(' Что ты хочешь с этим делать? (+, -, *, / доступны только)')
...
def Repeat_Or_No():
    ...
    while user_choice != 'Y' or user_choice != 'N':
        user_choice = input ( 'Вы хотите продолжить работу с комплексными номерами? (Y or N)')
        if user_choice == 'N':
            return False
        elif user_choice == 'Y':
            return True
        else:
            print('Недопустимый ответ!Вы хотите продолжить работу с комплексными номерами? Insert Y or N')
